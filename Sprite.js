...
Sprite.prototype.mover = function (map, dt) {
  ...
  if(this.frame>this.poses[this.pose].frames-1){
	...
	//Exibe as minas/tesouros/já visitado
	if(map.cells[this.gy][this.gx] == 3 || map.cells[this.gy][this.gx] == 4){
		if(map.cells[this.gy][this.gx] == 3){
			qtdTesourosEncontrados++;
		}
		map.cells[this.gy][this.gx] = 4; // tesouro
	}else if(map.cells[this.gy][this.gx] == 5 || map.cells[this.gy][this.gx] == 6){
		map.cells[this.gy][this.gx] = 6; // mina
		perdeu = true;
	}else{
		map.cells[this.gy][this.gx] = 2; // já visitado
	}
	
	var qtdMinas = 0;
	var qtdTesouros = 0;
	
	for(i = (this.gx-1); i <= (this.gx + 1); i++){
		for(j = this.gy-1; j <= (this.gy + 1); j++){
			if(map.cells[j][i] == 3){
				qtdTesouros++;
			}else if(map.cells[j][i] == 5){
				qtdMinas++;
			}
		}
	}

	document.getElementById("qtdMina").innerHTML= qtdMinas;
	document.getElementById("qtdTesouros").innerHTML= qtdTesouros;
